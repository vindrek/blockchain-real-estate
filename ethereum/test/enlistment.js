...
contract('Enlistment', async ([owner]) => {
  ...
  contract('Enlistment/contract creation', async ([deployerAddress]) => {
    ...
    before(async () => {
      contract = await ETC.new('landlord@email.xd', 'landlord name', 'Waker', 3, 2, 1, 15000, 58382794, 26734081, JSON.stringify(details));
    });
  ...
  contract('Offer flow', async () => {
    ...
    describe('Sending', async () => {
      ...
      before('create an enlistment and send offers', async () => {
        instance = await ETC.new('john@wick.xd', 'John Wick', 'Baker', 1, 2, 3, 45000, 58382794, 26734081, JSON.stringify(details));
  ...
  contract('Agreement contract flow', async () => {
    ...
    describe('Submitting drafts', async () => {
      ...
      beforeEach('create an enlistment, send an offer', async () => {
        instance = await ETC.new('johna@wick.xd', 'Johna Wick', 'Baker', 1, 2, 3, 45000, 58382794, 26734081, JSON.stringify(details));
        await instance.sendOffer(400, 'Cassian', 'cassian@reply.xd');
      });
